...
- **Sub-millisecond fuzzy search** over 2,200+ commands across 170 curated cheatsheets (recon, web, AD, cloud, C2, enumeration, exploit, post-exploit, passwords, network, system, vuln, wireless, plus DFIR/triage coverage: EVTX hunt, offline registry hives, MFT/USN/prefetch/LNK, VHDX/VMDK/E01 mount, Linux kernel crash dumps, browser history). Titles read like search queries — type `nmap scan full`, `kerberoast all users`, or `dump lsass remote`.
...
- **Author commands in-place** — `q new` walks you through adding a command (type a one-liner inline, or leave it blank to compose a multi-line command in `$EDITOR`): it auto-detects `{{vars}}` and asks each one's type/default, then title, description, risk/phase, and the target file (creating a new one if needed), and writes the markdown for you. `q edit [query]` edits or deletes an existing entry via an fzf picker — multi-line commands open in `$EDITOR`, so nothing gets flattened. No hand-writing cheatsheets.
...
├── cheatsheets/             # built-in markdown library (170 sheets · 2,200+ commands)
